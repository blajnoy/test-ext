      ...
      mutations.forEach(mutation => {
        if (mutation.type === 'childList') {
          for (let i = 0; i < mutation.addedNodes.length; i++) {
            ...
            if (this.hasClass(mutation.addedNodes[i], audioClassName)) {
              this.updateOneAudio(mutation.addedNodes[i]);
            } else {
              this.updateLinks(mutation.addedNodes[i]);
            }
            if (mutation.addedNodes[i].nodeType !== 1 && mutation.addedNodes[i].nodeType !== 9) continue;
          }
        }
      });
    },
